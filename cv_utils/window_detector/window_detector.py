...
class WindowFinder(Node):
	...
	def depth_image_callback(self, msg):
		...
		if cx and cy:
			# Normalizar coordenadas para o intervalo [-1, +1]
			# cx, cy já são relativos ao centro da imagem (podem ser negativos ou positivos)
			# Normalizar usando metade da largura/altura da imagem
			cx_normalized = float(cx) / (width / 2.0)  # Normalizado [-1, +1]
			cy_normalized = float(cy) / (height / 2.0) # Normalizado [-1, +1]
			
			# Publicar coordenadas normalizadas no sistema NED do drone
			centroid = Point()
			centroid.x = -1.0 * cy_normalized  # -cy_normalized (eixo x do drone = -y da imagem)
			centroid.y = cx_normalized         # cx_normalized (eixo y do drone = +x da imagem)
			centroid.z = 0.0
			self._centroid_publisher.publish(centroid)
			self._window_found_publisher.publish(True)
			...
			self._window_found_publisher.publish(False)
