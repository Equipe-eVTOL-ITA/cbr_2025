...
public:
    VisionNode() : Node("fase2_vision") {
        ...
        // Declare detection topic parameters for different detectors
        this->declare_parameter<std::string>("base_detection_topic", "/base_detector/detections");
        this->declare_parameter<std::string>("package_detection_topic", "/package_detector/detections");
        this->declare_parameter<bool>("enable_base_detection", true);
        this->declare_parameter<bool>("enable_package_detection", true);

        loadCameraAndModelParameters_();

        // Subscribe to base detections if enabled
        if (this->get_parameter("enable_base_detection").as_bool()) {
            std::string base_topic = this->get_parameter("base_detection_topic").as_string();
            base_detections_sub_ = this->create_subscription<vision_msgs::msg::Detection2DArray>(
                base_topic,
                vision_qos,
                [this](const vision_msgs::msg::Detection2DArray::SharedPtr msg) {
                    this->processDetections(msg, "landing_pad");
                }
            );
            RCLCPP_INFO(this->get_logger(), "Subscribed to base detections: %s", base_topic.c_str());
        }

        // Subscribe to package detections if enabled
        if (this->get_parameter("enable_package_detection").as_bool()) {
            std::string package_topic = this->get_parameter("package_detection_topic").as_string();
            package_detections_sub_ = this->create_subscription<vision_msgs::msg::Detection2DArray>(
                package_topic,
                vision_qos,
                [this](const vision_msgs::msg::Detection2DArray::SharedPtr msg) {
                    this->processDetections(msg, "package");
                }
            );
            RCLCPP_INFO(this->get_logger(), "Subscribed to package detections: %s", package_topic.c_str());
        }
    ...
    double lastBaseDetectionTime() {
        auto now = std::chrono::steady_clock::now();
        return std::chrono::duration<double>(now - base_detection_last_update_).count();
    }
    double lastPackageDetectionTime() {
        auto now = std::chrono::steady_clock::now();
        return std::chrono::duration<double>(now - package_detection_last_update_).count();
    }

    bool isThereDetection() {
        if (lastDetectionTime() > timeout_.count()) return false;
        return is_there_base_detection_ || is_there_package_detection_;
    }
    
    bool isThereBaseDetection() {
        if (lastBaseDetectionTime() > timeout_.count()) return false;
        return is_there_base_detection_;
    }
    
    bool isTherePackageDetection() {
        if (lastPackageDetectionTime() > timeout_.count()) return false;
        return is_there_package_detection_;
    }

    // Legacy methods for backward compatibility (return closest base)
    BoundingBox getClosestBbox() const { return closest_base_bbox_; }
    float getMinDistance() const { return min_base_distance_; }
    std::vector<BoundingBox> getDetections() const { return base_detections_; }
    
    // New specific methods
    BoundingBox getClosestBaseBbox() const { return closest_base_bbox_; }
    BoundingBox getClosestPackageBbox() const { return closest_package_bbox_; }
    float getMinBaseDistance() const { return min_base_distance_; }
    float getMinPackageDistance() const { return min_package_distance_; }
    std::vector<BoundingBox> getBaseDetections() const { return base_detections_; }
    std::vector<BoundingBox> getPackageDetections() const { return package_detections_; }
    std::vector<BoundingBox> getAllDetections() const { 
        std::vector<BoundingBox> all_detections = base_detections_;
        all_detections.insert(all_detections.end(), package_detections_.begin(), package_detections_.end());
    ...
    // Convenience method to publish base detection from closest detected base
    void publishClosestBaseDetection(const Eigen::Vector3d &drone_pos_w,
                                   const Eigen::Vector3d &drone_rpy_w,
                                   float mean_base_height = 0.0f,
                                   bool use_accurate_method = true) 
    {
        if (!isThereBaseDetection()) return;
        
        Eigen::Vector3d base_position;
        if (use_accurate_method) {
            base_position = getAccurateBase(drone_pos_w, drone_rpy_w, closest_base_bbox_);
        } else {
            base_position = getApproximateBase(drone_pos_w, drone_rpy_w, closest_base_bbox_, mean_base_height);
        }
        
        publishBaseDetection("landing_pad", base_position, closest_base_bbox_.confidence);
    }

    // New methods for package position estimation
    Eigen::Vector3d getApproximatePackage(const Eigen::Vector3d &drone_pos_w,
                                         const Eigen::Vector3d &drone_rpy_w,
                                         const BoundingBox &bbox,
                                         float mean_package_height = 0.1f)
    {
        return getApproximateBase(drone_pos_w, drone_rpy_w, bbox, mean_package_height);
    }

    Eigen::Vector3d getAccuratePackage(const Eigen::Vector3d &drone_pos_w,
                                      const Eigen::Vector3d &drone_rpy_w,
                                      const BoundingBox &bbox)
    {
        // Packages are typically smaller than bases, so we might need different handling
        // For now, use the same method but could be extended for package-specific logic
        return getAccurateBase(drone_pos_w, drone_rpy_w, bbox);
    }

    // Get position of closest package
    Eigen::Vector3d getClosestPackagePosition(const Eigen::Vector3d &drone_pos_w,
                                             const Eigen::Vector3d &drone_rpy_w,
                                             float mean_package_height = 0.1f,
                                             bool use_accurate_method = true)
    {
        if (!isTherePackageDetection()) {
            return Eigen::Vector3d::Zero();
        }
        
        if (use_accurate_method) {
            return getAccuratePackage(drone_pos_w, drone_rpy_w, closest_package_bbox_);
        } else {
            return getApproximatePackage(drone_pos_w, drone_rpy_w, closest_package_bbox_, mean_package_height);
        }
    }

    // Get position of closest base
    Eigen::Vector3d getClosestBasePosition(const Eigen::Vector3d &drone_pos_w,
                                          const Eigen::Vector3d &drone_rpy_w,
                                          float mean_base_height = 0.0f,
                                          bool use_accurate_method = true)
    {
        if (!isThereBaseDetection()) {
            return Eigen::Vector3d::Zero();
        }
        
        if (use_accurate_method) {
            return getAccurateBase(drone_pos_w, drone_rpy_w, closest_base_bbox_);
        } else {
            return getApproximateBase(drone_pos_w, drone_rpy_w, closest_base_bbox_, mean_base_height);
        }
    }
...
private:
    // ==== Members ============================================================
    rclcpp::Subscription<vision_msgs::msg::Detection2DArray>::SharedPtr base_detections_sub_;
    rclcpp::Subscription<vision_msgs::msg::Detection2DArray>::SharedPtr package_detections_sub_;
    
    // Separate detection storage
    std::vector<BoundingBox> base_detections_;
    std::vector<BoundingBox> package_detections_;
    
    // Timing for different detection types
    std::chrono::steady_clock::time_point detection_last_update_{};
    std::chrono::steady_clock::time_point base_detection_last_update_{};
    std::chrono::steady_clock::time_point package_detection_last_update_{};
    std::chrono::duration<double> timeout_{10.0};
    
    // Detection status
    bool is_there_base_detection_{false};
    bool is_there_package_detection_{false};

    // Closest detection tracking
    BoundingBox closest_base_bbox_{};
    BoundingBox closest_package_bbox_{};
    float min_base_distance_{0.0f};
    float min_package_distance_{0.0f};
    ...
    void processDetections(const vision_msgs::msg::Detection2DArray::SharedPtr msg, const std::string& expected_class_id) {
        // Update global detection timestamp
        detection_last_update_ = std::chrono::steady_clock::now();
        
        if (msg->detections.empty()) {
            if (expected_class_id == "landing_pad") {
                is_there_base_detection_ = false;
            } else if (expected_class_id == "package") {
                is_there_package_detection_ = false;
            }
            return;
        }

        // Parse detections
        std::vector<BoundingBox> new_detections;
        for (const auto& d : msg->detections) {
            ...
            if (!d.results.empty()) {
                ...
                b.confidence = 0.f;
                b.class_id   = expected_class_id;  // Use expected class if not provided
            }
            b.timestamp = static_cast<int64_t>(msg->header.stamp.sec) * 1000000000LL + msg->header.stamp.nanosec;
            
            // Only add if it matches expected class (allow for detector flexibility)
            if (b.class_id == expected_class_id || b.class_id.empty()) {
                b.class_id = expected_class_id;  // Ensure consistent class_id
                new_detections.push_back(b);
            }
        }

        // Update appropriate detection storage
        if (expected_class_id == "landing_pad") {
            base_detections_ = new_detections;
            if (!new_detections.empty()) {
                is_there_base_detection_ = true;
                base_detection_last_update_ = std::chrono::steady_clock::now();
            }
            computeClosestBaseBbox_();
        } else if (expected_class_id == "package") {
            package_detections_ = new_detections;
            if (!new_detections.empty()) {
                is_there_package_detection_ = true;
                package_detection_last_update_ = std::chrono::steady_clock::now();
            }
            computeClosestPackageBbox_();
        }
    }

    void computeClosestBaseBbox_() {
        if (base_detections_.empty()) {
            is_there_base_detection_ = false;
            return;
        }
        ...
        for (const auto& b : base_detections_) {
        ...
        closest_base_bbox_ = best_box;
        min_base_distance_ = best;
    }

    void computeClosestPackageBbox_() {
        if (package_detections_.empty()) {
            is_there_package_detection_ = false;
            return;
        }
        ...
        float best = 1e9f;
        BoundingBox best_box{};
        for (const auto& b : package_detections_) {
            // Work in normalized space regardless of input mode
            double cx = bbox_is_normalized_ ? b.center_x : b.center_x / static_cast<double>(image_width_);
            double cy = bbox_is_normalized_ ? b.center_y : b.center_y / static_cast<double>(image_height_);
            double d = (Eigen::Vector2d(cx, cy) - ic).norm();
        ...
        closest_package_bbox_ = best_box;
        min_package_distance_ = best;
    }
